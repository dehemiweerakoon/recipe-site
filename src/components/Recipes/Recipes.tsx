...
const Recipes = () => {
  ...
  function getSars(rating: number) {
    const fullStar = rating%1 >= 0.90 ? Math.ceil(rating) : Math.floor(rating);
                    ...
                    <div className="bg-white sm:max-w-full max-w-full rounded p-2">
                      <div className="flex flex-col justify-center items-center">
                        ...
                        <div className="flex flex-row gap-3 justify-center items-center">
                          ...
                          <div>
                            <h1 className="text-sm">reviews {item.reviewCount}</h1>
                          </div>
                        </div>
                        <hr className="border-t-2 border-gray-500 w-full mx-0 my-4" />
                        <div className="flex flex-row gap-10  items-center">
                          <h3 className="text-xs text-gray-500 ">{item.prepTimeMinutes+item.cookTimeMinutes} minutes</h3>
                          <button className="bg-[#DB9A9A] sm:px-5 sm:py-3 rounded-full text-xs font-semibold  px-3 py-3 cursor-pointer hover:bg-red-300"
                            onClick={() => {nav(`/recipes/${item.id}`)}}
                          >
                            Read More..
                          </button>
                        </div>
